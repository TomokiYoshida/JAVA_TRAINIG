...
/**
 * 課題2-4のデジタル時計で、属性をダイアログで指定できるようにしましたが、ダイアログを作
り直してください。
● レイアウトマネージャは、GridBagLayout を使用する。
● プロパティダイアログは、属性名＋のリストメニューが縦に並ぶようにする。
フォント フォントのリスト
フォントサイズ サイズのリスト
文字色 色のリスト（色が分かるようにカラーチップ1も表示すること）
背景色 色のリスト
...
public class DigitalClock extends JFrame implements Runnable {
	...
	public DigitalClock() {
		...
		characterColor = Integer.valueOf(prefsUtil.load("characterColorInteger", "0"));
	...
	class MyWindowListener implements WindowListener {
		...
		public void windowClosing(WindowEvent e) {// 閉じられている
			...
			String saveCharacterColor = String.valueOf(characterColor);
			...
			prefsUtil.save("characterColorInteger", saveCharacterColor);
	...
	class MyMenuActionListener implements ActionListener {
		...
		class PropertyDialog extends Dialog implements ActionListener {
			...
			void init() {
				c1.select(c1Index);
				c2.select(c2Index);
				j3.setSelectedIndex(c3indexInt);
				c4.select(c4Index);
				b3.setBackground(colors[c3indexInt]);
				b4.setBackground(colors[c4indexInt]);
			}
			...
			public void actionPerformed(ActionEvent e) {
				if (e.getActionCommand().equals("OK")) {
					...
					c1indexInt  = c1.getSelectedIndex();
					c2indexInt  = c2.getSelectedIndex();
					c3indexInt  = j3.getSelectedIndex();
					c4indexInt  = c4.getSelectedIndex();
			...
			class MyItemListener implements ItemListener {
				...
				public void itemStateChanged(ItemEvent e) {
					...
					font = fonts[c1.getSelectedIndex()].getName();
					fontSize = fontSizes[c2.getSelectedIndex()];
					characterColor = c3.getSelectedIndex();
					thisFrame.setBackground(colors[c4.getSelectedIndex()]);
					backGroundColor = colorsStr[c4.getSelectedIndex()];
